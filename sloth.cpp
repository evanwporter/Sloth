...
public:
    ...
    Series(Eigen::VectorXd values, std::shared_ptr<ObjectIndex> index) 
        : values_(std::move(values)), index_(std::move(index)), mask_(std::make_shared<slice<int>>(0, values_.size(), 1)) {}
...
public:
    ...
    DataFrame(MatrixXdRowMajor values, std::shared_ptr<ObjectIndex> index, std::shared_ptr<ColumnIndex> columns, std::shared_ptr<slice<int>> mask)
        : values_(std::move(values)), index_(std::move(index)), columns_(std::move(columns)), mask_(std::move(mask)) {}
...
PYBIND11_MODULE(sloth, m) {
    ...
    py::class_<DataFrame, std::shared_ptr<DataFrame>>(m, "DataFrame")
        .def(py::init<MatrixXdRowMajor, std::shared_ptr<ObjectIndex>, std::shared_ptr<ColumnIndex>, std::shared_ptr<slice<int>>>())
